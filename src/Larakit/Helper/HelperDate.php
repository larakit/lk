...
class HelperDate {
    ...
    static function fromDatetime($val, $time_format = 'H:i:s') {
        if (!$val) {
            return '';
        }
        return self::fromDate($val) . ', ' . date($time_format, strtotime($val));
    }

    static function fromDate($val) {
        if (!$val) {
            return '';
        }
    ...
    static function elapsed($sec) {
        ...
        //        dump(\LocalizedCarbon::now()->addSeconds((int)$sec)->diffForHumans(null, true));
        //        return \LocalizedCarbon::now()->addSeconds((int)$sec)->diffForHumans(null, true);
        return $ret;
    }
